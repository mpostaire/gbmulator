...
uint8_t mbc_read_eram(gb_t *gb, uint16_t address) {
    ...
    if (mbc->type == MBC7) {
        ...
        case 0x0080 /* 0xAx8x */:
            return mbc->mbc7.eeprom.pins;
        default:
            return 0xFF; // 0xAx0x -> 0xAx1x are write only, 0xAx7x and 0xAx9x -> 0xAxFx are unused and always reads 0xFF
        }
    }
    ...
    if (mbc->mbc3.rtc.enabled) { // mbc->mbc3.rtc.enabled implies that mmu->has_rtc is true
        ...
        case 0x08:
            return mbc->mbc3.rtc.latched_s & 0x3F;
        case 0x09:
            return mbc->mbc3.rtc.latched_m & 0x3F;
        case 0x0A:
            return mbc->mbc3.rtc.latched_h & 0x1F;
        case 0x0B:
            return mbc->mbc3.rtc.latched_dl;
        case 0x0C:
            return mbc->mbc3.rtc.latched_dh & 0xC1;
        default:
            return 0xFF;
        }
    }
...
void mbc_write_eram(gb_t *gb, uint16_t address, uint8_t data) {
    ...
    if (mbc->type == MBC7) {
        ...
        case 0x0080 /* 0xAx8x */:
            ...
            return;
        default:
            return; // 0xAx2x -> 0xAx5x are read only, 0xAx6x -> 0xAx7x and 0xAx9x -> 0xAxFx are unused
        }
    }
    ...
    if (mbc->eram_enabled && !can_access_rtc) {
        ...
        case 0x08:
            ...
            return;
        case 0x09:
            mbc->mbc3.rtc.m = data & 0x3F;
            return;
        case 0x0A:
            mbc->mbc3.rtc.h = data & 0x1F;
            return;
        case 0x0B:
            mbc->mbc3.rtc.dl = data;
            return;
        case 0x0C:
            mbc->mbc3.rtc.dh = data & 0xC1;
            return;
        default:
            return;
        }
