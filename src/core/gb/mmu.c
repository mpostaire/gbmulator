...
int parse_header_mbc_byte(uint8_t mbc_byte, uint8_t *mbc_type, uint8_t *has_eram, uint8_t *has_battery, uint8_t *has_rtc, uint8_t *has_rumble) {
    ...
    if (mbc_type)
        *mbc_type = tmp_mbc_type;
    if (has_eram)
        *has_eram = tmp_has_eram;
    if (has_battery)
        *has_battery = tmp_has_battery;
    if (has_rtc)
        *has_rtc = tmp_has_rtc;
    if (has_rumble)
...
int mmu_reset(gb_t *gb, const uint8_t *rom, size_t rom_size) {
    ...
    // gb->mmu.rom_bankn_addr = 0; // initialized to MMU_ROM_BANKN - ROM_BANK_SIZE = 0 by memset
    // gb->mmu.eram_bank_addr = 0; // initialized to 0 by memset
    gb->mmu.wram_bankn_addr_offset = -MMU_WRAM_BANK0;
    gb->mmu.vram_bank_addr_offset  = -MMU_VRAM;

    // gb->mmu.mbc7.accelerometer.latched_x = 0x8000;
    // gb->mmu.mbc7.accelerometer.latched_y = 0x8000;
    // gb->mmu.mbc7.accelerometer.latch_ready = 1;

    if (gb->mmu.mbc.type == CAMERA) {
        memset(&gb->mmu.mbc.camera.sensor_image, 0, sizeof(gb->mmu.mbc.camera.sensor_image));
    } else if (gb->mmu.mbc.type == MBC7) {
        gb->mmu.mbc.mbc7.accelerometer.latched_x = 0x81D0;
...
static inline uint8_t read_io_register(gb_t *gb, uint8_t io_reg_addr) {
    ...
    case IO_SB:
        return mmu->io_registers[io_reg_addr];
    case IO_SC:
        return mmu->io_registers[io_reg_addr] | (gb->cgb_mode_enabled ? 0x7C : 0x7E);
    case 0x03:
        return 0xFF;
    case IO_DIV:
        return gb->timer.div_timer >> 8;
    case IO_TIMA:
        return mmu->io_registers[io_reg_addr];
    case IO_TMA:
        return mmu->io_registers[io_reg_addr];
    case IO_TAC:
        return mmu->io_registers[io_reg_addr] | 0xF8;
    case 0x08 ... IO_IF - 1:
        return 0xFF;
    case IO_IF:
        return mmu->io_registers[io_reg_addr] | 0xE0;
    case IO_NR10:
        return mmu->io_registers[io_reg_addr] | 0x80;
    case IO_NR11:
        return mmu->io_registers[io_reg_addr] | 0x3F;
    case IO_NR12:
        return mmu->io_registers[io_reg_addr];
    case IO_NR13:
        return 0xFF;
    case IO_NR14:
        return mmu->io_registers[io_reg_addr] | 0xBF;
    case IO_NR20:
        return 0xFF;
    case IO_NR21:
        return mmu->io_registers[io_reg_addr] | 0x3F;
    ...
    case IO_NR24:
        return mmu->io_registers[io_reg_addr] | 0xBF;
    case IO_NR30:
        return mmu->io_registers[io_reg_addr] | 0x7F;
    case IO_NR31:
        return 0xFF;
    case IO_NR32:
        return mmu->io_registers[io_reg_addr] | 0x9F;
    case IO_NR33:
        return 0xFF;
    case IO_NR34:
        return mmu->io_registers[io_reg_addr] | 0xBF;
    ...
    case IO_NR51:
        return mmu->io_registers[io_reg_addr];
    case IO_NR52:
        return mmu->io_registers[io_reg_addr] | 0x70;
    case 0x27 ... IO_WAVE_RAM - 1:
        return 0xFF;
    ...
    case IO_KEY0:
        return 0xFF;
    case IO_KEY1:
        return gb->cgb_mode_enabled ? (mmu->io_registers[io_reg_addr] | 0x7E) : 0xFF;
    case 0x4E:
        return 0xFF;
    case IO_VBK:
        return gb->base->opts.mode == GBMULATOR_MODE_GBC ? 0xFE | (mmu->io_registers[io_reg_addr] & 0x01) : 0xFF;
    case IO_BANK:
        return 0xFF;
    case IO_HDMA1 ... IO_HDMA4:
        return 0xFF;
    case IO_HDMA5:
        return gb->cgb_mode_enabled ? mmu->io_registers[io_reg_addr] : 0xFF;
    case IO_RP:
    ...
    case 0x57 ... 0x67:
        return 0xFF;
    case IO_BGPI:
        return gb->base->opts.mode == GBMULATOR_MODE_GBC ? mmu->io_registers[io_reg_addr] | 0x40 : 0xFF;
    case IO_BGPD: {
    ...
    case 0x6C ... 0x6F:
        return 0xFF;
    case IO_SVBK:
        return gb->cgb_mode_enabled ? 0xF8 | (mmu->io_registers[io_reg_addr] & 0x07) : 0xFF;
    case 0x71:
        return 0xFF;
    case 0x72 ... 0x73:
        return gb->base->opts.mode == GBMULATOR_MODE_GBC ? mmu->io_registers[io_reg_addr] : 0xFF;
    case 0x74:
        return gb->cgb_mode_enabled ? mmu->io_registers[io_reg_addr] : 0xFF;
    case 0x75:
        return gb->base->opts.mode == GBMULATOR_MODE_GBC ? mmu->io_registers[io_reg_addr] | 0x8F : 0xFF;
    case IO_PCM12:
    case IO_PCM34:
...
static inline void write_io_register(gb_t *gb, uint8_t io_reg_addr, uint8_t data) {
    ...
    case IO_TAC:
        ...
        switch (data & 0x03) {
        case 0x00:
            gb->timer.tima_increase_div_bit = 9;
            break;
        case 0x01:
            gb->timer.tima_increase_div_bit = 3;
            break;
        case 0x02:
            gb->timer.tima_increase_div_bit = 5;
            break;
        case 0x03:
            gb->timer.tima_increase_div_bit = 7;
            break;
        }
        break;
    case IO_NR10:
...
SERIALIZED_SIZE_FUNCTION(gb_mmu_t, mmu, SERIALIZED_MEMBERS {
...
SERIALIZER_FUNCTION(gb_mmu_t, mmu, SERIALIZED_MEMBERS {
        gb_mbc_t *tmp = &gb->mmu.mbc;
        MBC_SERIALIZED_MEMBERS })
#undef W
#undef Z
