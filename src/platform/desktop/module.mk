...
LIBS := libadwaita-1 manette-0.2 opengl openal gstreamer-1.0

override CFLAGS += $(shell pkg-config --cflags $(LIBS))
override LDLIBS += $(shell pkg-config --libs $(LIBS))
...
$(ICONS): images/icons/$(BINNAME).svg
	...
	magick -background none -density 1200 -filter Lanczos $^ -resize $(patsubst $(ICONDIR)/%/$(BINNAME).png,%,$@) $(ICONDIR)/$(patsubst $(ICONDIR)/%/$(BINNAME).png,%,$@)/$(BINNAME).png
...
install: $(ODIR_STRUCTURE) $(ICONS) $(BIN)
	...
	install -m 0644 images/icons/$(BINNAME).svg /usr/share/icons/hicolor/scalable/apps
